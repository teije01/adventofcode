...
a, and b are on. Because 4 is the only digit that uses four segments, eafb means that to render a
...
def decode_segment_data(segment: str) -> str:
    """Sort segment sections and split input and output"""
    seg_input, seg_output = segment.split(" | ")
    decypher = decypher_input_segment(seg_input)
    seg_output = "".join([decypher["".join(sorted(digit))] for digit in seg_output.split()])
    return seg_output


def decypher_input_segment(input_segment: str) -> dict[str, str]:
    input_cypher_list = ["".join(sorted(digit)) for digit in input_segment.split()]
    cypher_lengths = [len(digit) for digit in input_cypher_list]
    cypher_1 = input_cypher_list[cypher_lengths.index(2)]
    cypher_4 = input_cypher_list[cypher_lengths.index(4)]
    cypher_7 = input_cypher_list[cypher_lengths.index(3)]
    cypher_8 = input_cypher_list[cypher_lengths.index(7)]

    sec_cf = set(cypher_1)
    sec_bd = set(cypher_4).difference(sec_cf)
    sec_eg = set(cypher_8).difference(sec_bd.union(cypher_7))

    cypher_0, = [*filter(lambda x: len(x) == 6 and sec_bd.intersection(x) != sec_bd,
                         input_cypher_list)]
    cypher_2, = [*filter(lambda x: len(x) == 5 and sec_eg.intersection(x) == sec_eg,
                         input_cypher_list)]
    cypher_3, = [*filter(lambda x: len(x) == 5 and sec_cf.intersection(x) == sec_cf,
                         input_cypher_list)]
    cypher_5, = [*filter(lambda x: len(x) == 5 and sec_bd.intersection(x) == sec_bd,
                         input_cypher_list)]
    cypher_6, = [*filter(lambda x: len(x) == 6 and sec_cf.intersection(x) != sec_cf,
                         input_cypher_list)]
    cypher_9, = [*filter(lambda x: len(x) == 6 and sec_eg.intersection(x) != sec_eg,
                         input_cypher_list)]
    decypher_dict = {
        cypher_0: "0",
        cypher_1: "1",
    ...
    return decypher_dict


if __name__ == "__main__":

    with open("solutions/2021/day8/input.txt", "r") as f:
        segment_numbers = [decode_segment_data(segment) for segment in f.read().splitlines()]

    counter = Counter()
    for number in segment_numbers:
        counter.update(number)

    print(f"Answer 1: {counter['1'] + counter['4'] + counter['7'] + counter['8']}")
    print(f"Answer 2: {sum(map(int, segment_numbers))}")
