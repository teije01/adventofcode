...
packet are represented as binary with the most significant bit first. For example, a version
...
So, this packet represents a literal value with binary representation 011111100101, which is 2021
in decimal.
...
more interesting:
    ...
    the values of their sub-packets. If they only have a single sub-packet, their value is the
...
@dataclass
class Packet(ABC):
    """General packet data structure"""
    version: int
    packet_type: int

    @abstractmethod
    def version_sum(self):
        """Return the version sum of (nested) packet object(s)"""

    @abstractmethod
    def packet_value(self):
        """Return the value of the packet (operator)"""


@dataclass
class Literal(Packet):
    """Literal packet with a value"""
    value: int
    ...
    def version_sum(self):
        """Return the version sum of nested packet objects"""
        return self.version
...
class Operator(Packet):
    ...
    def __post_init__(self):
        self.operator = OPERATOR_MAP[self.packet_type]

    def __repr__(self):
        return f"{self.operator.__name__}({self.sub_packets})"

    def version_sum(self):
        """Return the version sum of nested packet objects"""
        return self.version + sum(sub_packet.version_sum() for sub_packet in self.sub_packets)

    def packet_value(self):
        """Return the value of the packet"""
        if self.packet_type < 4:
            return self.operator(sub_packet.packet_value() for sub_packet in self.sub_packets)
        return self.operator(self.sub_packets[0].packet_value(), self.sub_packets[1].packet_value())


def read_packet(binary_str: str) -> tuple[Packet, int]:
    """Read next packet from binary_str and return operator with the number of bits read"""
    version = int(binary_str[:3], 2)
    packet_type = int(binary_str[3:6], 2)
    bits_read = 6

    if packet_type != 4:  # Operator
        length_type = bool(int(binary_str[bits_read]))
        bits_read += 1
        sub_packets = []

        if length_type:  # defines number of sub packets
            number_of_sub_packets = int(binary_str[bits_read:bits_read+11], 2)
            bits_read += 11
            for packet_number in range(number_of_sub_packets):
                packet, sub_bits_read = read_packet(binary_str[bits_read:])
                sub_packets.append(packet)
                bits_read += sub_bits_read

        else:  # defines number of bits to read as sub packets
            bits_to_read = int(binary_str[bits_read:bits_read+15], 2)
            bits_read += 15  # after reading all packets
            while bits_to_read > 0:
                packet, sub_bits_read = read_packet(binary_str[bits_read:])
                sub_packets.append(packet)
                bits_to_read -= sub_bits_read
                bits_read += sub_bits_read
        packet = Operator(version, packet_type, length_type, sub_packets)

    else:  # Literal
        packet_value = ""
        while True:
            packet_value += binary_str[bits_read+1:bits_read+5]
            if binary_str[bits_read] == "0":
                bits_read += 5
                break
            bits_read += 5
        packet_value = int(packet_value, 2)
        packet = Literal(version, packet_type, packet_value)
...
if __name__ == "__main__":
    ...
    with open("solutions/2021/day16/input.txt", "r") as f:
        data = f.read()

    binary = f"{int(data, 16):b}"
    packet_, bits_read_ = read_packet(binary)

    print(packet_, "==", packet_.packet_value())
    print(f"Answer 1: {packet_.version_sum()}")
    print(f"Answer 2: {packet_.packet_value()}")
