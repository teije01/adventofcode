...
So, the last completion string above - ])}> - would be scored as follows:
    ...
    Multiply the total score by 5 to get 0, then add the value of ] (2) to get a new total score
    of 2.

    Multiply the total score by 5 to get 10, then add the value of ) (1) to get a new total score
    of 11.

    Multiply the total score by 5 to get 55, then add the value of } (3) to get a new total score
    of 58.

    Multiply the total score by 5 to get 290, then add the value of > (4) to get a new total score
    of 294.
...
BRACKETS = {"[": "]", "<": ">", "(": ")", "{": "}"}
IllegalChar = Enum("IllegalChar", {")": 3, "]": 57, "}": 1197, ">": 25137})
CompleteChar = Enum("IllegalChar", {")": 1, "]": 2, "}": 3, ">": 4})


def calc_illegal_char_score(line, autocomplete=False) -> int:
    """Return the illegal char value or 0 if not illegal"""
    tracker = []
    for char in line:
        if char in BRACKETS:
            tracker.append(char)
        elif char == BRACKETS[tracker[-1]]:
            tracker.pop(-1)
        else:
            return IllegalChar[char].value
    return calc_autocomplete_score(tracker if autocomplete else [])


def calc_autocomplete_score(tracker):
    """Return the autocomplete score"""
    score = 0
    for char in tracker[::-1]:
        score = score * 5 + CompleteChar[BRACKETS[char]].value
    return score


if __name__ == "__main__":

    with open("solutions/2021/day10/input.txt", "r") as f:
        syntax_lines = f.read().splitlines()

    invalid_syntax_scores = [calc_illegal_char_score(line) for line in syntax_lines]
    autocomplete_scores = [
        calc_illegal_char_score(line, autocomplete=True)
        for line, invalid_score in zip(syntax_lines, invalid_syntax_scores)
        if invalid_score == 0
    ]

    print(f"Answer 1: {sum(invalid_syntax_scores)}")
    print(f"Answer 2: {np.median(autocomplete_scores)}")
