...
Here are some examples of a single explode action:
    ...
    [7,[6,[5,[4,[3,2]]]]] becomes [7,[6,[5,[7,0]]]] (the 2 has no regular number to its right, and
    so it is not added to any regular number).

    [[6,[5,[4,[3,2]]]],1] becomes [[6,[5,[7,0]]],3].

    [[3,[2,[1,[7,3]]]],[6,[5,[4,[3,2]]]]] becomes [[3,[2,[8,0]]],[9,[5,[4,[3,2]]]]] (the pair [3,2]
    is unaffected because the pair [7,3] is further to the left; [3,2] would explode on the next
    action).
...
snailfish numbers are each listed on a separate line. Add the first snailfish number and the
second, then add that result and the third, then add that result and the fourth, and so on until
all numbers in the list have been used once.
...
To check whether it's the right answer, the snailfish teacher only checks the magnitude of the
final sum. The magnitude of a pair is 3 times the magnitude of its left element plus 2 times the
...
class SnailFish:
    def __init__(self, number: list[any]):
        self.number = number
        self.tree = nx.Graph

    def __add__(self, other: 'SnailFish'):
        return SnailFish([self.number, other.number])
    ...
    def reduce(self):
        ...
        self.number = reduce(self)

    def add_explosion(self, explosion, path):
        ...

    def magnitude(self):
        """Return the SnailFish magnitude"""
        ...


class ReduceBaseEvent(Exception):
    """Exception for a reduce event"""


class Explode(ReduceBaseEvent):
    """Stop recursion with explode event"""
...
class Split(ReduceBaseEvent):
    """Stop recursion with split event"""


def explode_or_split(number: list[Union[list, int]], path=None, method="explode"):
    """Explode or split nested list"""
    if path is None:
        path = [0]
    else:
        path.append(0)
    if isinstance(number, list):
        for i, snail in enumerate(number):
            path[-1] = i
            if method == "explode" and len(path) == 4 and isinstance(snail, list):
                raise Explode(number.pop(i), path)
            elif method == "split" and isinstance(snail, int) and snail > 9:
                number[i] = [snail//2, snail-snail//2]
                raise Split()
            elif isinstance(snail, list):
                explode_or_split(snail, path)
        path.pop(-1)


def reduce(snailfish: SnailFish):
    """Reduce operation on a snailfish"""
    mutation = True
    while mutation:
        try:
            explode_or_split(snailfish.number, method="explode")
            explode_or_split(snailfish.number, method="split")
            mutation = False
        except Explode as e:
            snailfish.add_explosion(e.number, e.path)
        except Split:
            pass
    return snailfish.number


snailfish1 = SnailFish([[[[4,3],4],4],[7,[[8,4],9]]])
snailfish_sum = snailfish1 + SnailFish([1,1])
snailfish_sum.reduce()


if __name__ == "__main__":

    with open("solutions/2021/day18/input.txt", "r") as f:
        data = f.read()
