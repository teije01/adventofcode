...
ten spaces marked 1 through 10 clockwise. Each player's starting space is chosen randomly (your
...
moving clockwise on spaces in order of increasing value, wrapping back around to 1 after 10). So,
if a player is on space 7 and they roll 2, 2, and 1, they would move forward 5 times, to spaces 8,
...
always splits the universe into three copies: one where the outcome of the roll was 1, one where it
...
start_player_pattern = re.compile(r"Player (?P<player_id>\d) starting position: (?P<starting_position>\d{1,2})")
...
class Player:
    ...
    def __repr__(self):
        return f"player {self.player_id} at {self.position} with score={self.score}"

    def move(self, steps: int, spaces: int):
        """Move player and update score"""
        self.position = (self.position + steps - 1) % spaces + 1
        self.score += self.position


class DiracDice:
    """The dirac dice game"""
    def __init__(self, players: list[Player], spaces: int):
    ...
    def __repr__(self):
        return str(self.current_player_idx) + "".join(repr(player) for player in self.players)

    def get_current_player(self, pass_turn=True) -> Player:
        """Get current player and move turn to next player"""
        player = self.players[self.current_player_idx]
        self.current_player_idx = (self.current_player_idx + pass_turn) % len(self.players)
        return player

    def play(self, die: Iterator[int], max_score=1000, rolls_per_turn=3):
        """Play the game until a maximum score is reached"""
        while self.max_score < max_score:
            player = self.get_current_player()
            player.move(sum(islice(die, rolls_per_turn)), spaces=self.spaces)
            self.rolls += rolls_per_turn
        self.is_finished = True

    def multiverse_step(self, die: Iterator[int], max_score=21) -> list["DiracDice", ...]:
        """Play a single round and return multiverse instances"""
        outcomes = []
        for roll in die:
            game = self.copy()
            player = game.get_current_player()
            player.move(roll, spaces=self.spaces)
            game.rolls += 1
            if game.max_score >= max_score:
                game.is_finished = True
            outcomes.append(game)
        return outcomes

    @property
    def max_score(self):
        """Maximum score"""
        return max(self.players, key=lambda p: p.score).score
    ...
    def __hash__(self):
        ...
        player_stats = chain.from_iterable((player.position, player.score) for player in dd.players)
        return hash((self.current_player_idx, ) + tuple(player_stats))


def deterministic_100_sided_die():
    """returns roll number, increases next roll by 1 and wraps at 100 to 1"""
    roll = 1
    while True:
        yield roll
        roll = roll % 100 + 1
...
if __name__ == "__main__":
    ...
    with open("solutions/2021/day21/test_input.txt", "r") as f:
        data = f.read()

    players = [Player(int(m['player_id']), int(m["starting_position"])) for m in re.finditer(start_player_pattern, data)]
    dd = DiracDice(players, spaces=10)
    dd_multi = dd.copy()
    dd.play(die=deterministic_100_sided_die())

    multiverse_counter = Counter(dd_multi.multiverse_step(die=[1, 2, 3], max_score=21))
    move_score = 0
    while not all(game.is_finished for game in multiverse_counter.keys()):
        step_counter = Counter()
        while multiverse_counter:
            game, count = multiverse_counter.popitem()
            if game.max_score != move_score or game.is_finished:
                step_counter.update({game: count})
            step_counter.update(dict.fromkeys(game.multiverse_step([1, 2, 3], max_score=21), count))
        move_score += 1
        multiverse_counter.update(step_counter)

    print(f"Answer 1: {min(players, key=lambda p: p.score).score * dd.rolls}")
    print(f"Answer 2: {''}")
