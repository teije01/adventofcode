...
STACKS_TYPE = dict[int, list[str]]


def read_stacks(stacks_txt: str) -> STACKS_TYPE:
    """Read stacks from input txt and convert to dict of lists indexed by position int"""
    stack_ids, *layers = reversed(stacks_txt.split("\n"))
    stack_dict = {int(stack_id): [] for stack_id in stack_ids[1::4]}
    for layer in layers:
        layer_crates = layer[1::4]
        for i, crate in enumerate(layer_crates, 1):
            if crate != " ":
                stack_dict[i].append(crate)
    return stack_dict


def update_stacks(stacks: STACKS_TYPE, rearrangement_procedure_txt: str, multi_crates=False):
    """Update stack based on rearrangement procedure"""
    move_pattern = re.compile("move (?P<move>\d+) from (?P<from>\d) to (?P<to>\d)")
    for move_match in re.finditer(move_pattern, rearrangement_procedure_txt):
        crates_to_move = [
            stacks[int(move_match["from"])].pop(-1) for _step in range(int(move_match["move"]))
        ]
        if multi_crates:
            crates_to_move.reverse()
        stacks[int(move_match["to"])].extend(crates_to_move)


if __name__ == "__main__":

    with open("solutions/2022/day5/input.txt", "r") as f:
        stacks_input, rearrangement_procedure_input = f.read().split("\n\n")

    stacks_dict_1 = read_stacks(stacks_input)
    stacks_dict_2 = deepcopy(stacks_dict_1)
    update_stacks(stacks_dict_1, rearrangement_procedure_input, multi_crates=False)
    update_stacks(stacks_dict_2, rearrangement_procedure_input, multi_crates=True)

    print(f"Solution 1: {''.join(stacks_dict_1[stack][-1] for stack in stacks_dict_1)}")
    print(f"Solution 2: {''.join(stacks_dict_2[stack][-1] for stack in stacks_dict_2)}")
